...
### Write (trust >= 1)
`shadow_repo_add`, `shadow_repo_remove`, `shadow_contact_add`, `shadow_contact_remove`, `shadow_system_add`, `shadow_system_remove`, `shadow_memory_teach`, `shadow_memory_forget`, `shadow_suggest_accept`, `shadow_suggest_dismiss`, `shadow_observation_ack`, `shadow_observation_resolve`, `shadow_observation_reopen`, `shadow_profile_set`, `shadow_focus`, `shadow_available`, `shadow_events_ack`
...
- **33 MCP tools** (15 read + 17 write + check_in) — includes observation lifecycle (ack/resolve/reopen)
- **4 hooks** (SessionStart, PostToolUse, UserPromptSubmit, Stop)
- **Status line** with emojis (activity, mood, energy, trust badge, suggestions, heartbeat countdown)
- **Daemon** via launchd with graceful shutdown (SIGTERM → close web server + DB + free port)
- **Dashboard** React app at localhost:3700 (16 routes) with sidebar badges, markdown rendering
- **Observation lifecycle** — votes/dedup, status (active/acknowledged/resolved/expired), enriched context (repo, branch, files, sessions), auto-resolve by LLM
- **Suggestion pipeline** — accept creates Run, plan generated by LLM, execute/session/discard/executed-manual states, feedback loop (dismiss notes + accept patterns fed back to suggest prompt)
- **Runs** — trust-gated execution (plan at L2, execute at L3+), worktree support, session creation (`claude --resume`), archive
- **Heartbeat stats** — llm_calls, tokens_used, events_queued, memories_promoted/demoted persisted per heartbeat
- **Async CLI adapter** — `spawn` instead of `spawnSync`, web server stays responsive during LLM calls
- **Smart analyze** — receives active observations (no duplication), dismiss feedback, auto-resolves stale observations
- **Smart suggest** — no operational suggestions, dedup against pending, learns from accept/dismiss patterns
- **Morning page** — daily brief with metrics, last heartbeat, new memories, runs to review, suggestions, observations
- **Deep linking** — `?highlight=<id>` across observations, suggestions, runs pages with auto-scroll + pulse
- **Memory traceability** — memories linked to source heartbeat via `source_id`
...
- **Trust score**: ~20 (level 2 advisor, suggestions active)
